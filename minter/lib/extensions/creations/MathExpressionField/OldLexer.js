    ...
    tokenizeTex(texStr) {
        ...
        while (i < length) {
            ...
            if (!this.isAlpha(c) && c in lexemeToType) {
                ...
                if (nextChar === '\\') {
                    // double backslash
                    type = 16 /* TokenType.Dblbackslash */;
                    lexeme = '\\\\';
                }
                else if (nextChar === ' ') {
                    // space character: ignore
                    type = 51 /* TokenType.Space */;
                    lexeme = '\\ ';
                }
                else {
                    // TeX command
            ...
            if (type !== 51 /* TokenType.Space */) {
                tokens.push(new Token(lexeme, type, i));
            }
            i += lexeme.length;
        }
        tokens.push(new Token('EOF', 40 /* TokenType.Eof */, i));
        return tokens;
    }
    defaults() { return {}; }
    mutabilities() { return {}; }
}
//# sourceMappingURL=OldLexer.js.map